...
-- Add LOP to initialize_leave_balances
...
BEGIN
  ...
  VALUES
    ...
    (NEW.user_id, 'lop', 0, 0, _academic_year)
...
-- Fix deduction: ignore selected leave type, always CL→EL→ML→LOP
...
BEGIN
  ...
  -- If still remaining after CL+EL+ML, add to LOP
  IF _remaining > 0 THEN
    UPDATE public.leave_balances
    SET used = used + _remaining
    WHERE user_id = NEW.user_id AND leave_type = 'lop' AND academic_year = _academic_year;
    
    -- If no LOP row exists, create one
    IF NOT FOUND THEN
      INSERT INTO public.leave_balances (user_id, leave_type, opening, used, academic_year)
      VALUES (NEW.user_id, 'lop', 0, _remaining, _academic_year)
      ON CONFLICT (user_id, leave_type, academic_year) DO UPDATE SET used = leave_balances.used + _remaining;
    END IF;
  END IF;
